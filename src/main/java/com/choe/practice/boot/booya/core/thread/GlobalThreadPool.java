...
public class GlobalThreadPool {
  ...
  private static Logger log = LoggerFactory.getLogger(GlobalThreadPool.class);
  ...
  synchronized public static void init() {
    ...
    executor = new ThreadPoolExecutor(0, Integer.MAX_VALUE, 60L, TimeUnit.SECONDS,
        new SynchronousQueue<Runnable>(), BooyaThreadFactory.create("global", false));
    log.info("GlobalThreadPool init complete!");
  }

  /**
   * 关闭全局线程池
   *
   * @param isNow 是否立即关闭（不等待正在执行的线程）
   */
  synchronized public static void shutdown(boolean isNow) {
    ...
    log.info("GlobalThreadPool shutdown complete!");
  }
